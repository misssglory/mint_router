...
async fn process_query(query_str: &str, db: &DatabaseManager) -> String {
    ...
    match parts[0] {
        "GET" => {
            if parts.len() < 3 {
                return json!({"error": "Invalid GET command. Usage: GET <type> <value>"}).to_string();
            }
            ...
            match parts[1] {
                "mint" => {
                    ...
                    if unique_chats {
                        match db.get_unique_chats_for_mint(mint).await {
                            ...
                            Err(err) => json!({"error": format!("Failed to query chats: {}", err)}).to_string(),
                        }
                    } else {
                        match db.get_messages_by_mint(mint, use_cache).await {
                            ...
                            Err(err) => json!({"error": format!("Failed to query messages: {}", err)}).to_string(),
                        }
                    }
                }
                "chat:id" => {
                ...
                "stats" => {
                    match db.get_stats().await {
                        ...
                        Err(err) => json!({"error": format!("Failed to get stats: {}", err)}).to_string(),
                    }
                }
